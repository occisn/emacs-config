...
^General hydra:
^---------------
Go: [d]: open downloads directory           | [e]: open en-cours (on-going) directory
    [x]: open Dropbox directory in dired   | [m]: ielm
    [s]: open pro1 App1                    | [l]: local-repos
...
Tabs: C-x t 2 for new tab | M-x tab-bar-rename-tab | C-x t RET to switch | C-x t 0 to close
Buffers: [f]: (from) open dired buffer corresponding to Windows path copied to clipboard
...
Find and grep --> dired hydra or Project hydra
Mini-buffer : C-q C-j to go to a new line | ivy : C-M-j to force
Text: [k]: show kill ring (xah-show-kill-ring) | To search and insert Unicode: C-x 8 RET or M-x counsel-unicode-char
      ...
      search in file: C-s or M-x occur [ C-u C-s for native search ] ||| M-x replace-regexp (example: ^abc.*$)
...
"
 ...
 (defun context-hydra-launcher ()
   ...
   (cond
    ...
    (pcase (my-init--dired-project-type)
      ('c (hydra-dired-c/body))
